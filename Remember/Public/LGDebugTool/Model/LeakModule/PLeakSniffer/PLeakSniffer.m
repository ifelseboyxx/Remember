...
- (void)detectPong:(NSNotification*)notif
{
    ...
    //we got a leak here
    if (_useAlert) {
        NSString* msg = [NSString stringWithFormat:@"%@", [leakedObject class]];
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"检测到可能有泄漏" message:msg delegate:nil cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
        [alertView show];
    }
